...
package{'php71w-mcrypt':
  ensure  => present,
  require => Exec['Intall WebTatic'],
} ->

package{'php71w-mbstring':
  ensure  => present,
  require => Exec['Intall WebTatic'],
} ->

package{'php71w-mysql':
  ensure  => present,
  require => Exec['Intall WebTatic'],
} ->
...
wget::fetch { "Download phing phar 2.15.2":
  source      => 'http://www.phing.info/get/phing-2.15.2.phar',
  destination => '/libs/',
  timeout     => 0,
  verbose     => false,
} ->
